	...
	getDmSys: function() {
		...
		$.ajax({
			...
			success: function(data) {
				...
				return _this.inTheQueue ? isOver === !0 ? (_this.stopQueue(), alert("鎶辨瓑锛屽晢鍝佸凡鍞絼锛�"), !1) : (_this.getDmTimer && clearInterval(_this.getDmTimer), void(token ? _this.getShopCart(token) : (_this.inTheQueue || _this.startQueue(), _this.getDmTimer = setInterval(function() {
					_this.getDmSys()
				},
				speed)))) : !1
			}
...
XIAOMI.app.miniCart = {
	...
	loadingStr: '<p class="loading">璐墿杞︿腑杩樻病鏈夊晢鍝侊紝璧剁揣閫夎喘鍚э紒</p>',
	speed: 500,
	init: function() {
		var _this = this,
		timeOut = null;
...
XIAOMI.app.navCategory = function() {
	...
	function init() {
		...
		if ($("#J_categoryContainer").find(".nav-category-list").html(templateHtml), addSrcset($(".nav-category-list")), isCategoryStatic) toggleCategoryList("static");
			...
			$categoryTriggerBtn.append('<i class="glyphicon"></i>');
			var timeoutCategory;
...
XIAOMI.app.setLoginInfo = {
	...
	updateDom: function() {
		var displayName = "";
		displayName = this.data.userName.length > 7 ? this.data.userName.substr(0, 7) + "鈥�": this.data.userName;
		var timeoutUserMenu, html = '娆㈣繋鎮� <b class="user-name">' + displayName + '<span class="arrow"></span></b><span class="sep">|</span><a href="' + this.config.orderSite + '/user/order">鎴戠殑璁㈠崟</a><span class="sep">|</span><a href="http://account.xiaomi.com/" target="_blank">鎴戠殑灏忕背璐︽埛</a>',
		$userInfoMenu = $('<div class="user-info-menu"><ul><li><a href="http://my.mi.com/portal" target="_blank">涓汉涓績</a></li><li><a href="' + XIAOMI.GLOBAL_CONFIG.orderSite + '/user/favorite">鎴戠殑鏀惰棌</a></li><li><a href="' + this.config.logoutUrl + '">閫€鍑虹櫥褰�</a></li></ul><span class="menu-arrow"></span></div>');
